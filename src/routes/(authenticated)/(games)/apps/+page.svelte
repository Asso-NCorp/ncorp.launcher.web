...
{#snippet title()}
    <BlurFade delay={0.3} class="text-3xl font-bold">Liste des applications</BlurFade>
{/snippet}
{#snippet head()}
...
{#if global.gamesDisplayMode === "grid"}
    <ScrollArea class="flex-1">
        <GamesGrid games={GamesStore.games.filter((game) => game.useNotifications === false)} />
    </ScrollArea>
{:else}
    <ScrollArea class="flex-1">
        <GamesDataTable
            games={GamesStore.games.filter((game) => game.useNotifications === false)}
