...
export const load: PageServerLoad = async ({ locals }) => {
    ...
    }, zod(profileFormSchema as any), { allowFiles: true });
...
export const actions: Actions = {
    updateProfile: async ({ request, locals }) => {
        ...
        const form = await superValidate(formData, zod(profileFormSchema as any));
        ...
        if (removeAvatar) {
            ...
            if (locals.user.image) { // Only attempt removal if an image is currently set
                ...
                try {
                    ...
                    if (userAvatarFiles.length > 0) {
                        for (const oldFile of userAvatarFiles) {
                            ...
                            logger.info(`Deleted avatar file ${oldFilePath} for user ${userId} due to removal request.`);
                        }
                    ...
                    logger.warn(`Failed to delete avatar files for user ${userId} during removal request:`, cleanupError);
                    // Set avatarUrl to null to reflect user's intent, even if file deletion failed.
                    avatarUrl = null;
                }
            } else {
                logger.info(`User ${userId} requested avatar removal, but no avatar was set. Setting avatarUrl to null.`);
        ...
        try {
            ...
            }); logger.info('Update user response:', updatedUser); if (updatedUser) {
